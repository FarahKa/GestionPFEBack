 ...
 */
    ...
    async updatePFE(new_pfe: UpdatePFEDto) {
        ...
        if (new_pfe.private)
            pfe.private = new_pfe.private
        if (new_pfe.rapport)
            pfe.rapport = new_pfe.rapport
        if (new_pfe.state)
            pfe.state = new_pfe.state
        if (new_pfe.subject)
            pfe.subject = new_pfe.subject
        if (new_pfe.valid)
            pfe.valid = new_pfe.valid
        ...
        return await this.studentRepository.find(
            ...
            for (let index = 0; index < students.length; index++) {
                if (students[index].soutenance) {
                    ...
                    if (yes)
                        data.push(students[index].soutenance.pfe)
                }
            }
            return data
        })
